...
typedef enum { IfK, RepeatK, AssignK, ReadK, WriteK, ProgramK, TypeK, WhileK } StmtKind;
typedef enum { OpK, ConstIntK, IdK, ConstStrK} ExpKind;
...
extern const char* errorMsg[MAX_ERROR];
...
TreeNode* stmt_sequence(void)
{
 ...
 //       (token != ELSE) && (token != UNTIL) && (token != WHILE))  ////stmt_seqçš„followé›† 
        while (token == SEMI)   
    {
        TreeNode* q;
        match(SEMI);
...
TreeNode* varlist()
{
    TreeNode* prev = NULL, * first = newExpNode(IdK);
    if ((first != NULL) && (token == ID))
        first->attr.name = copyString(tokenString);
    match(ID);
    prev = first;
    while (token == COMMA) {
        ...
        prev->child[0] = temp;
        prev = temp;
        if ((temp != NULL) && (token == ID))
            temp->attr.name = copyString(tokenString);
        match(ID);
    }
...
TreeNode* decl()
{
    TreeNode* ret = newStmtNode(TypeK);  //TypeKÊÇ·ñÒ»¶¨ÐèÒª£¿ 
    switch (token) {
    case INT:
        ret->attr.name = copyString(tokenString);
        match(INT);
        ret->child[0] = varlist();// varlist ¹ÒÔÚÏÂÃæ 
        break;
    case BOOL:
        ret->attr.name = copyString(tokenString);
        match(BOOL);
...
//////add by zp,return the last node;
TreeNode* declarations()
{
    TreeNode* ret = NULL, * prev=NULL;

    while ((token == INT) | (token == BOOL) | (token == STRING)) {
        TreeNode* temp = decl();  //¹Ò½Úµã 
        if (ret == NULL) {
            ret = temp;
            prev = temp;
        }
...
TreeNode* parse(void)
{
    TreeNode* t=NULL, * root=NULL;     
    root = newStmtNode(ProgramK);
    token = getToken();
